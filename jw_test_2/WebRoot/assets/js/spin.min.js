 ...
 */
(function(a, b) { if (typeof exports == "object") { module.exports = b() } else { if (typeof define == "function" && define.amd) { define(b) } else { a.Spinner = b() } } }(this, function() {
    ...
    function h(r) { for (var q = 1, s = arguments.length; q < s; q++) { r.appendChild(arguments[q]) } return r }
    var i = (function() {
    ...
    function m(u, v) {
        ...
        for (r = 0; r < e.length; r++) { q = e[r] + v; if (t[q] !== undefined) { return q } }
        if (t[v] !== undefined) { return v }
    }

    function f(q, s) { for (var r in s) { q.style[m(q, r) || r] = s[r] } return q }

    function k(s) { for (var q = 1; q < arguments.length; q++) { var r = arguments[q]; for (var t in r) { if (s[t] === undefined) { s[t] = r[t] } } } return s }

    function j(q) { var r = { x: q.offsetLeft, y: q.offsetTop }; while ((q = q.offsetParent)) { r.x += q.offsetLeft, r.y += q.offsetTop } return r }
    ...
    function b(q) {
        if (typeof this == "undefined") { return new b(q) }
    ...
    k(b.prototype, {
        spin: function(z) {
            ...
            var D = this,
                r = D.opts,
                s = D.el = f(g(0, { className: r.className }), { position: r.position, width: 0, zIndex: r.zIndex }),
    ...
    function l() {
        function q(s, r) { return g("<" + s + ' xmlns="urn:schemas-microsoft.com:vml" class="spin-vml">', r) }
        ...
        b.prototype.lines = function(v, u) {
            ...
            var w = -(u.width + u.length) * 2 + "px",
                z = f(A(), { position: "absolute", top: w, left: w }),
                y;
            ...
            if (u.shadow) { for (y = 1; y <= u.lines; y++) { x(y, -2, "progid:DXImageTransform.Microsoft.Blur(pixelradius=2,makeshadow=1,shadowopacity=.3)") } }
            for (y = 1; y <= u.lines; y++) { x(y) }
            return h(v, z)
        };
    ...
    if (!m(a, "transform") && a.adj) { l() } else { o = m(a, "animation") }
    return b
}));