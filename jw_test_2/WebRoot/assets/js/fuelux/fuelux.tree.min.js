...
(function(b, d) {
    ...
    var c = function(f, e) {
        ...
        this.$element.on("click", ".tree-item", b.proxy(function(g) { this.selectItem(g.currentTarget) }, this));
        this.$element.on("click", ".tree-folder-header", b.proxy(function(g) { this.selectFolder(g.currentTarget) }, this));
        this.render()
    };
    c.prototype = {
        constructor: c,
            ...
            this.options.dataSource.data(g.data(), function(i) {
                ...
                b.each(i.data, function(j, m) {
                    ...
                    if (m.type === "folder") {
                        ...
                        n.data(m);
                        if ("icon-class" in m) { n.find("i").addClass(m["icon-class"]) }
                    } else {
                        if (m.type === "item") {
                    ...
                    if (g.hasClass("tree-folder-header")) { h.find(".tree-folder-content:eq(0)").append(k) } else { g.append(k) }
                });
        ...
        selectItem: function(g) {
            ...
            var h = [];
            if (this.options.multiSelect) { b.each(i, function(k, l) { var j = b(l); if (j[0] !== f[0]) { h.push(b(l).data()) } }) } else {
                if (i[0] !== f[0]) {
            ...
            if (f.hasClass("tree-selected")) {
                ...
                if (this.options.multiSelect) { h.push(f.data()) }
            }
            if (h.length) { this.$element.trigger("selected", { info: h }) }
        ...
        selectFolder: function(g) {
            ...
            if (m.find(k).length) {
                ...
                h.show();
                if (!f.children().length) { this.populate(m) }
            } else {
                ...
                h.hide();
                if (!this.options.cacheItems) { h.empty() }
            }
        ...
        selectedItems: function() {
            ...
            var e = [];
            b.each(f, function(g, h) { e.push(b(h).data()) });
            return e
        },
        collapse: function() {
    ...
    b.fn.tree = function(g) { var f = Array.prototype.slice.call(arguments, 1); var h; var e = this.each(function() { var k = b(this); var j = k.data("tree"); var i = typeof g === "object" && g; if (!j) { k.data("tree", (j = new c(this, i))) } if (typeof g === "string") { h = j[g].apply(j, f) } }); return (h === d) ? e : h };
    b.fn.tree.defaults = { multiSelect: false, loadingHTML: "<div>Loading...</div>", cacheItems: true };
    b.fn.tree.Constructor = c;
    b.fn.tree.noConflict = function() { b.fn.tree = a; return this }
})(window.jQuery);